...
function organizeCards(playerCards) {
    ...
    for (let color of ['black'].concat(colors)) {
        ...
        organizedCards = organizedCards.concat(filteredCards.sort((a, b) => a.symbol.localeCompare(b.symbol)));
    }
    return organizedCards;
}

function startTable() {
    let startingCardIndex = deck.findIndex(card => !isNaN(parseInt(card.symbol)));
    table.push(deck.splice(startingCardIndex, 1)[0]);
}
...
function renderDeck() {
    ...
    switch (carta.symbol) {
        ...
        case "p2":
        case "p4":
            symbolHTML = `<p>+${carta.symbol.substr(1)}</p>`;
            break;
        default:
            symbolHTML = `<p>${carta.symbol}</p>`;
    }
...
function renderTable() {
    ...
    let angle = (numCards === 0 || carta.symbol === "p4" || carta.symbol === "changeColor") ? 0 : (Math.random() - 0.5) * 2 * 15;
    let symbolHTML;
    switch (carta.symbol) {
        case "reverse":
            symbolHTML = '<ion-icon name="refresh-outline"></ion-icon>';
    ...
    `;
    if (numCards === 6) {
        tableCardObject.children[0].remove();
    }
